...
@pytest.mark.spark
def test_parent_refresh_pass_does_not_write_dq_records(
    spark: SparkSession, wired: None
) -> None:
    """The parent-refresh pass must merge parents without owning DQ accounting.

    run_orders_conform_with_parent_refresh drains products and customers CDF
    through a SEPARATE checkpoint so orders' FK check sees current parents. That
    means the parent CDF is consumed twice — once by the parent's own trigger
    job, once by the refresh — and because quarantine and dq_metrics are
    append-only, both entities were counted twice.

    Measured on CE: the products quarantine held 47 rows under each of two
    silver_run_ids for one bronze batch, and dq_metrics carried six runs for
    products and customers against three for orders.

    Silver itself was correct throughout (MERGE on the key is idempotent); only
    the DQ report was inflated. The refresh exists for read consistency, so it
    merges but records nothing. The parent's own job still processes the same
    CDF from its own checkpoint and quarantines the bad rows exactly once.
    """
    from silver.main import process_conform_batch

    batch = spark.createDataFrame(
        [
            _row(20, 200, 2, "b1"),      # clean
            _row(21, None, 2, "b1"),     # completeness failure
        ],
        schema=_bronze_orders_cdf_schema(),
    )

    rows_read, rows_written, rows_quarantined = process_conform_batch(
        spark, "orders", batch, "run-refresh", "de_assessment", None, record_dq=False
    )

    assert rows_read == 2
    assert rows_written == 1, "valid parent rows must still reach silver"
    assert rows_quarantined == 0, "the refresh pass records no quarantine"

    assert spark.table(QUARANTINE_TBL).count() == 0
    assert spark.table(METRICS_TBL).count() == 0
